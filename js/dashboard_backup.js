...
function updateUsersStats() {
    ...
    const onlineUsers = Math.floor(activeUsers * 0.6); // Simulação
...
function openUserModal(userId = null) {
    ...
    if (userId) {
        ...
        // Mostrar campos de senha na criação
...
// Atualizar opções do relatório
function updateReportOptions() {
    const reportType = document.getElementById('reportType')?.value;
    
    // Atualizar período baseado no tipo
    const periodSelect = document.getElementById('reportPeriod');
    if (periodSelect && reportType === 'estoque') {
        // Para estoque, remover opções de período muito curtas
...
// Inicializar ao carregar módulos específicos
